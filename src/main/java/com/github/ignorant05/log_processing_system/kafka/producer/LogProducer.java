...
public class LogProducer implements AutoCloseable {
	...
	public void sendSync(LogEntry logEntry) throws Exception {
		...
		try {
			...
			if (messagesSent.get() % 100 == 0) {
				System.out.printf("Sent %d messages to %s-%d (offset: %d)%n", messagesSent.get(), metadata.topic(),
	...
	public void sendAsync(LogEntry logEntry) throws JsonProcessingException, ExecutionException, InterruptedException {
		String json = JsonUtil.toJson(logEntry);
		...
		producer.send(record, (metadata, exception) -> {
			if (exception == null) {
				...
				messagesFailed.incrementAndGet();
				System.err.printf("Failed to send message Asynchranously: %s%n", exception.getMessage());
			}
		});
	}
